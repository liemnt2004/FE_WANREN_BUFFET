...
export async function getAllOrderDetailsByOrderId(orderId: number): Promise<OrderDetailModel[]>{
    ...
    try {
        const data = await request(`https://wanrenbuffet.online/api-data/Orders/${orderId}/orderDetails`);
...
export async function getTableNumberByOrderId(orderId: number): Promise<number>{
    try {
        const response = await request(`https://wanrenbuffet.online/api-data/Orders/${orderId}/tablee`);
