...
export async function getAllPromotion(): Promise<PromotionModel[]> {
    ...
    try {
        const data = await request('https://wanrenbuffet.online/api-data/Promotion/search/findByPromotionStatus?promotionStatus=true');
        console.log(data)
...
export async function GetPromotionById(PromotionId: number): Promise<PromotionModel | null> {
    try {
        ...
        const data = await request(`https://wanrenbuffet.online/api-data/Promotion/${PromotionId}`);
