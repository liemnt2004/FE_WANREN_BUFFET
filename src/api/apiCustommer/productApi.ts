...
export async function getAllProduct(): Promise<ProductModel[]> {
    ...
    try {
        const data = await request('https://wanrenbuffet.netlify.app/Product');
...
export async function getProductHot(): Promise<ProductModel[]> {
    try {
        const productIds: number[] = await request('https://wanrenbuffet.netlify.app/api/product/ProductHot'); // Đảm bảo đúng endpoint
...
export async function fetchProductsByType(typeFood: string): Promise<ProductModel[]> {
    ...
    try {
        ...
        const data = await request(`https://wanrenbuffet.netlify.app/api/product/by-category?categoryName=${typeFood}`);
...
export async function getOrderDetail(OrderDetailId:number): Promise<ProductModel[]> {
    const rs: ProductModel[] = [];
    try {
        const data = await request(`https://wanrenbuffet.netlify.app/Order_detail/${OrderDetailId}/product`);
        // Spring Data REST thường trả về dữ liệu trong _embedded
        if (data && data._embedded && data._embedded.products) {
            for (const product of data._embedded.products) {
                const productModel = new ProductModel(
...
export async function SearchProduct(foodname: string): Promise<ProductModel[]> {
    const rs: ProductModel[] = [];
    try {
        const data = await request(`https://wanrenbuffet.netlify.app/Product/search/findByProductNameContaining?productName=${foodname}`);
        // Spring Data REST thường trả về dữ liệu trong _embedded
        if (data && data._embedded && data._embedded.products) {
            for (const product of data._embedded.products) {
                const productModel = new ProductModel(
...
export async function fetchProductsByCategory(category: string): Promise<ProductModel[]> {
    ...
    try {
        // Gọi API
        const response = await fetch(`https://wanrenbuffet.netlify.app/api/product/by-category?categoryName=${category}`, {
            method: "GET",
            headers: {
